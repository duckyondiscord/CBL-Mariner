...
Name:           php-sebastian-exporter4
Version:        4.0.5
Release:        1%{?dist}
Summary:        Export PHP variables for visualization, version 4
...
require_once '%{php_home}/SebastianBergmann/RecursionContext4/autoload.php';
EOF


%install
mkdir -p   %{buildroot}%{php_home}/SebastianBergmann
cp -pr src %{buildroot}%{php_home}/SebastianBergmann/Exporter4
...
for cmd in php php74 php80 php81 php82; do
  if which $cmd; then
    $cmd -d auto_prepend_file=%{buildroot}%{php_home}/SebastianBergmann/Exporter4/autoload.php \
...
%doc README.md
%doc composer.json
%{php_home}/SebastianBergmann/Exporter4
